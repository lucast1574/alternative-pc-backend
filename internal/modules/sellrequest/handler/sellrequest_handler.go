package handler

import (
	"context"
	"net/http"
	"time"

	"github.com/alternative/backend/internal/modules/sellrequest/model"
	"github.com/alternative/backend/internal/platform/database"
	"github.com/gin-gonic/gin"
	"go.mongodb.org/mongo-driver/bson"
	"go.mongodb.org/mongo-driver/bson/primitive"
	"go.mongodb.org/mongo-driver/mongo/options"
)

func CreateSellRequest(c *gin.Context) {
	var input model.CreateSellRequestInput
	if err := c.ShouldBindJSON(&input); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	// Calculate estimate
	estimates, ok := model.PriceEstimates[input.PartType]
	if !ok {
		estimates = model.PriceEstimates[model.PartOther]
	}

	req := model.SellRequest{
		PartType:    input.PartType,
		Brand:       input.Brand,
		Model:       input.Model,
		Condition:   input.Condition,
		Description: input.Description,
		SellerName:  input.SellerName,
		SellerEmail: input.SellerEmail,
		SellerPhone: input.SellerPhone,
		EstimateMin: estimates[0],
		EstimateMax: estimates[1],
		Status:      model.SellPending,
		CreatedAt:   time.Now(),
		UpdatedAt:   time.Now(),
	}

	// If user is authenticated, link it
	if userID, exists := c.Get("userId"); exists {
		objID, err := primitive.ObjectIDFromHex(userID.(string))
		if err == nil {
			req.UserID = objID
		}
	}

	coll := database.DB.Collection("sell_requests")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	result, err := coll.InsertOne(ctx, req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "error al crear solicitud"})
		return
	}

	req.ID = result.InsertedID.(primitive.ObjectID)

	c.JSON(http.StatusCreated, gin.H{
		"request":     req,
		"estimateMin": req.EstimateMin,
		"estimateMax": req.EstimateMax,
		"message":     "Solicitud recibida. Revisaremos tu parte y te contactaremos con una oferta.",
	})
}

func GetSellRequests(c *gin.Context) {
	coll := database.DB.Collection("sell_requests")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	opts := options.Find().SetSort(bson.D{{Key: "createdAt", Value: -1}})
	cursor, err := coll.Find(ctx, bson.M{}, opts)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "error al obtener solicitudes"})
		return
	}
	defer cursor.Close(ctx)

	var requests []model.SellRequest
	if err := cursor.All(ctx, &requests); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "error al decodificar"})
		return
	}

	if requests == nil {
		requests = []model.SellRequest{}
	}

	c.JSON(http.StatusOK, requests)
}

func UpdateSellRequest(c *gin.Context) {
	id := c.Param("id")
	objID, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "id inválido"})
		return
	}

	var input struct {
		Status        model.SellRequestStatus `json:"status"`
		OfferedAmount float64                 `json:"offeredAmount"`
		AdminNotes    string                  `json:"adminNotes"`
	}
	if err := c.ShouldBindJSON(&input); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	update := bson.M{"updatedAt": time.Now()}
	if input.Status != "" {
		update["status"] = input.Status
	}
	if input.OfferedAmount > 0 {
		update["offeredAmount"] = input.OfferedAmount
	}
	if input.AdminNotes != "" {
		update["adminNotes"] = input.AdminNotes
	}

	coll := database.DB.Collection("sell_requests")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	_, err = coll.UpdateOne(ctx, bson.M{"_id": objID}, bson.M{"$set": update})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "error al actualizar"})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "solicitud actualizada"})
}

func GetPriceEstimates(c *gin.Context) {
	estimates := make(map[string]map[string]float64)
	for partType, prices := range model.PriceEstimates {
		estimates[string(partType)] = map[string]float64{
			"min": prices[0],
			"max": prices[1],
		}
	}
	c.JSON(http.StatusOK, estimates)
}
